...
* We’re going to stay in a '''binary world''' in this lecture
...
* We don’t have to be limited to accuracy based on algorithm purposes
...
* '''Pareto Optimal''' solutions are usually ones that we’re trying to reach but nondominating points are those that we have already reached
...
* A measure of how ‘alone’ a point is in a distribution shows whether it has a high crowding distance– if it is more alone, and far away from other points on its front, then it has a higher crowding distance and we choose that point as the winner of the tiebreaker because we want to explore the areas near it and see if we can achieve a better point from there
...
We’ll have a multi-week project regarding EMADE
...
Now we’re moving into automated machine learning, not just ML
...
<pre>$ conda env create –f emade.yml</pre>
...
* We use the <code>–w</code> flag along with your peer’s server info in the dbconfig in order to allow your computer to act as a worker process for your peer’s master process.
* the <code>–w</code> says don’t run the master process– only run the evaluations
...
EMADE Structure
* We’ll have to make our own database/tables
